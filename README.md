...
**候補の出し方**: 対象機器の IP アドレスと**完全一致**するレコードを提示します。同じプライベート IP が別の顧客にも存在するため、**候補が 1 件でも自動選択はしません**。顧客名・機器名・アカウント名・備考を確認して選んでください。対象は NW 機器の区分（Router / L2 Switch / 基幹L3SW / YamahaRTX など）に限定され、サーバや SaaS のアカウントは出てきません。
