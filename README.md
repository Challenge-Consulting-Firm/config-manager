...
> - 都度入力したパスワード・enablePassword はヘルパーとの通信にのみ使われ、**BFF には送信されません**（取得したコンフィグ本文のみを SPA → BFF へ送ります）。
> - ヘルパーは `127.0.0.1` のみで待ち受け、許可 Origin（`PUBLIC_BASE_URL` + 開発用 localhost）以外からの要求は拒否します。

#### 登録済み認証情報の自動適用（省略可）

Kintone の「顧客情報（ノード管理）」アプリに機器のアカウント名・パスワードが登録済みであれば、取得ダイアログに候補として提示し、手入力を省けます。`KINTONE_CUSTOMER_INFO_APP_ID` / `KINTONE_CUSTOMER_INFO_APP_TOKEN` を設定すると有効になり、未設定なら従来どおり都度入力のみです。

**候補の出し方**: 対象機器の IP アドレスと**完全一致**するレコードを提示します。同じプライベート IP が別の顧客にも存在するため、**候補が 1 件でも自動選択はしません**。顧客名・機器名・アカウント名・システム種別・備考を確認して選んでください。`対象` が `削除・管理外` のレコードは出しません。

同一機器に複数アカウントが登録されている場合（`root` と `admin` など）は、その全部が候補に並びます。アカウント名で選び分けてください。

**平文パスワードの流れ**: ブラウザには渡りません。

```
SPA  --(取得ボタン)-->  BFF: 一回限りトークンを発行（平文は返さない）
SPA  --(トークン)-->    ヘルパー
ヘルパー --(トークン)--> BFF: /helper/credentials/redeem で平文を引き換え
ヘルパー --(平文)-->    機器へログイン
```

トークンは 32 バイトの乱数・一回限り・60 秒で失効し、発行時の利用者と対象機器に束縛されます。この設計のため、ヘルパーは BFF と通信します（Issue #43 時点の「ヘルパーは BFF に接続しない」方針からの変更点です）。引き換え先はヘルパーが検証済みの Origin から組み立てるため、SPA が任意の URL を指定することはできません。

> **事前準備**: 作業履歴アプリの `action` ドロップダウンに `credential` の選択肢が必要です。`node scripts/setup-kintone.mjs --app audit` を実行して同期してください。無いと監査ログを書けず、本機能は 503 で停止します（下記のとおり fail closed のため）。

> **セキュリティ注意**:
> - 参照は**すべて監査ログに記録されます**（`action=credential`）。記録されるのは利用者・対象機器・アカウント名・結果で、パスワードとトークンは記録しません。**監査を書けない場合は認証情報を渡しません**（fail closed）。
> - Telnet で登録済み認証情報を使う場合は、平文送信への同意チェックが必須です。中央保管された管理者パスワードが LAN 上を流れるため、SSH を優先してください。
> - `KINTONE_CUSTOMER_INFO_APP_TOKEN` は**レコード閲覧のみ**の権限で発行してください。本機能はこのアプリへ書き込みを一切行いません。
> - Kintone の API トークンはレコード単位の閲覧権限をバイパスします。参照元アプリをレコード単位で制限して運用している場合は、config-manager 経由でその制限を越えて参照できる点に注意してください。
> - 参照元アプリには Excel からの貼り付けに由来するゼロ幅スペースが混入したレコードがあります。**パスワードは既定では加工せずそのまま送ります**。認証に失敗する場合のみ、ダイアログの「不可視文字を除去して送信する」を使ってください（恒久的にはアプリ側のデータ修正が必要です）。
