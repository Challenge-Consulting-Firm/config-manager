...
ヘルパーは利用者の PC 上で動くネイティブバイナリです。配布経路がすり替えられると、LAN 内で任意コードを実行され、機器の認証情報を扱う立場も奪われます。**未署名の配布物は公開しない**ことを前提にします（Issue #79）。

- Windows は Authenticode 署名、macOS は Developer ID 署名 + Apple 公証を `Release Helper` ワークフロー内で行う。署名・公証・検証のいずれかが失敗すると公開ジョブに進まない（fail closed）
- 公開ジョブは再ビルドせず、署名済み artifact から checksum を計算する。「署名対象＝公開対象」を担保するのはこの一点なので、ビルド順序を変えないこと
- 成果物には GitHub Artifact Attestations による provenance を付ける（`gh attestation verify` で検証可能）
- SPA の導線から SmartScreen / Gatekeeper の迂回手順（`xattr -d com.apple.quarantine`、「詳細情報 → 実行」）を削除し、**SHA-256 を自動照合して一致した場合だけ起動する**コマンドを提示する。警告が出た場合は実行させず管理者へエスカレーションする
- `latest.json` の `signature` が `none`（または未記載）の場合、SPA は「未署名。使用しないでください」と表示する

鍵の保管・ローテーション・失効時の運用手順は [`HELPER-RELEASE.md`](HELPER-RELEASE.md) を参照。
